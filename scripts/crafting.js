import {
  ...
  localise,
  MODULE_NAME,
  ROLLOPTION_ITEM_PREFIX,
  ROLLOPTION_PREFIX,
  ROLLOPTION_SETTINGS_PREFIX,
  setPreferredPayMethod,
} from "./constants.js";
import {
  projectBeginDialog,
  projectCraftDialog,
 ...
 */
export async function beginAProject(
  crafterActor,
  itemDetails,
  skipDialog = true
) {
  if (!itemDetails.UUID || itemDetails.UUID === "") {
    console.error(
      "[HEROIC CRAFTING AUTOMATION] Missing UUID when beginning a project!"
    );
    return;
  }
  ...
  let dialogResult = {};
  if (!skipDialog) {
    dialogResult = await projectBeginDialog(
      itemDetails,
      getPreferredPayMethod(crafterActor)
    );
  } else {
    dialogResult = {
      startingProgress: 0,
  ...
  const payment = payWithCoinsAndTrove(
    ...
    new game.pf2e.Coins({
      cp: dialogResult.startingProgress,
    })
  );
  ...
  if (!payment.canPay) {
    ui.notifications.warn(
      localise("ProjectBeginWindow.CannotPay", {
  ...
  if (payment.removeCopper > 0) {
    await crafterActor.inventory.removeCoins({
      cp: payment.removeCopper,
    });
  }
  ...
  ChatMessage.create({
    ...
    speaker: {
      alias: crafterActor.name,
    },
  });

  await crafterActor.update({
    [`flags.${MODULE_NAME}.projects`]: actorProjects.concat(newProjects),
  });
}
 ...
 */
export async function craftAProject(
  crafterActor,
  itemDetails,
  skipDialog = true,
  projectOwner = crafterActor
) {
  if (!itemDetails.UUID || itemDetails.UUID === "") {
    console.error(
      "[HEROIC CRAFTING AUTOMATION] Missing Item UUID when crafting a project!"
    );
    return;
  }
  if (!itemDetails.projectUUID || itemDetails.projectUUID === "") {
    console.error(
      "[HEROIC CRAFTING AUTOMATION] Missing Project UUID when crafting a project!"
    );
    return;
  }
  ...
  const project = actorProjects.filter(
    (project) => project.ID === itemDetails.projectUUID
  )[0];

  if (!project) {
    ui.notifications.error(
      localise("CraftWindow.DoesNotHaveProjectToCraft", {
        name: projectOwner.name,
        projectUUID: itemDetails.projectUUID,
      })
    );
    return;
  }
  ...
  for (const toggle in dialogResult.toggles) {
    if (dialogResult.toggles.hasOwnProperty(toggle)) {
      rushCosts = rushCosts.plus(
        game.pf2e.Coins.fromString(dialogResult.toggles[toggle].rushCost)
      );
    }
  }
  ...
  if (!payment.canPay) {
    ui.notifications.warn(
      localise("CraftWindow.CannotPay", {
        name: crafterActor.name,
      })
    );
    return;
  ...
  if (
    project.progressInCopper + dialogResult.spendingAmount.copperValue >=
    cost.copperValue
  ) {
    ChatMessage.create({
      user: game.user.id,
      content: localise("CraftWindow.Progress.SkipCheck", {
        name: crafterActor.name,
    ...
    await rollCraftAProject(crafterActor, project, {
      duration: dialogResult.duration,
      overtime: dialogResult.overtime,
      craftingMaterials: dialogResult.spendingAmount,
      rushCosts,
      modifiers: dialogResult.modifiers,
      toggles: dialogResult.toggles,
      projectOwner,
    });
  }
}
...
async function rollCraftAProject(crafterActor, project, details) {
  ...
  for (const rollMod of details.modifiers) {
    if (rollMod.active) {
      ...
      if (rollMod.toggledBy) {
        settingsRollOptions.push(
          `${ROLLOPTION_SETTINGS_PREFIX}:${rollMod.toggledBy}`
        );
      }
    }
  }
  ...
  for (const toggle in details.toggles) {
    if (details.toggles.hasOwnProperty(toggle)) {
      ...
      if (
        elem.value &&
        !settingsRollOptions.includes(`${ROLLOPTION_SETTINGS_PREFIX}:${toggle}`)
      ) {
  ...
  const craftSkillCheck = crafterActor.skills[skillName].extend({
    ...
    rollOptions: [
      `action:craft`,
      `action:craftproj`,
      `${ROLLOPTION_PREFIX}:duration:${details.duration}`,
      ...settingsRollOptions,
      ...itemRollOptions,
    ],
      ...
      if (message instanceof ChatMessage) {
        ...
        if ("AddCraftProgress" in crafterActor.synthetics) {
          crafterActor.synthetics["AddCraftProgress"].forEach((synthetic) => {
            if (
              typeof synthetic.outcome === "undefined" ||
              synthetic.outcome.length === 0 ||
              synthetic.outcome.includes(outcome)
            ) {
              if (synthetic.coins) {
                craftDetails.progressMade = craftDetails.progressMade.plus(
                  synthetic.coins
                );
              } else {
                switch (synthetic.mode) {
                  case "multiply":
                    craftDetails.progressMade = craftDetails.progressMade.scale(
                      synthetic.amount
                    );
                    break;
                }
        ...
        const flavour = await renderTemplate(
          `modules/${MODULE_NAME}/templates/crafting-result.hbs`,
          craftDetails
        );
        message.updateSource({
          flavor: message.flavor + flavour,
        });
...
export async function abandonProject(crafterActor, projectUUID) {
  ...
  await crafterActor.update({
    [`flags.${MODULE_NAME}.projects`]: actorProjects.filter(
...
export async function editProject(crafterActor, projectUUID) {
  if (!projectUUID || projectUUID === "") {
    console.error(
      "[HEROIC CRAFTING AUTOMATION] Missing Project UUID when editing a project!"
    );
    return;
  }
  ...
  if (!project) {
    ui.notifications.error(
      localise("CharSheet.CannotEditProject", {
        name: crafterActor.name,
        projectUUID,
      })
    );
  ...
  project.progressInCopper =
    dialogResult.progressInCopper < 0
      ? project.progressInCopper
      : dialogResult.progressInCopper;
  project.DC = dialogResult.DC < 0 ? project.DC : dialogResult.DC;
  project.batchSize =
    dialogResult.batchSize <= 0 ? project.batchSize : dialogResult.batchSize;
...
export async function getProjectsToDisplay(crafterActor) {
  ...
  const projectItems = await Promise.all(
    projects.map(async (project) => {
      const projectItem = await fromUuid(project.ItemUUID);
      const cost = game.pf2e.Coins.fromPrice(
        projectItem.price,
        project.batchSize
      );
 ...
 * @returns
 */
export async function progressProject(
  crafterActor,
  projectUUID,
  hasProgressed,
  amount
  ...
  if (!project) {
    ui.notifications.error(
      localise("CraftWindow.DoesNotHaveProjectToProgress", {
        name: projectOwner.name,
        projectUUID: itemDetails.projectUUID,
      })
    );
  ...
  if (hasProgressed) {
    ...
    if (project.progressInCopper >= cost.copperValue) {
      ...
      const result = crafterActor.isOwner
        ? await crafterActor.addToInventory(itemObject, undefined)
        : "permissionLacking";

      if (!result) {
        ui.notifications.warn(
          game.i18n.localize("PF2E.Actions.Craft.Warning.CantAddItem")
        );
        return;
      }
      ...
      if (result === "permissionLacking") {
        ChatMessage.create({
          user: game.user.id,
          content: localise("CraftWindow.Progress.Finish", {
            name: crafterActor.name,
            batchSize: project.batchSize,
            itemName: projectItem.name,
          }).concat(
            localise("CraftWindow.Progress.LacksPermissionToFinish", {
              name: crafterActor.name,
      ...
      ChatMessage.create({
        ...
        content: `${localise("CraftWindow.Progress.Progress", {
          ...
          goal: cost.toString(),
                ...
                <progress id="remainingMaterialPercent" value="${
    ...
    if (project.progressInCopper <= 0) {
      ChatMessage.create({
        user: game.user.id,
        content: localise("CraftWindow.Progress.FatalSetback", {
          name: crafterActor.name,
          batchSize: project.batchSize,
          itemName: projectItem.name,
        }),
